...
export default async function Product(props: Props) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* --- BREADCRUMBS ala FILE SYSTEM --- */}
      <nav className="flex items-center gap-2 mb-8 font-mono text-[10px] font-bold uppercase tracking-tighter text-text-disabled overflow-x-auto no-scrollbar">
        <Link href="/" className="hover:text-primary-main whitespace-nowrap">
          root
        </Link>
        <span>/</span>
        <Link
          href="/search"
          className="hover:text-primary-main whitespace-nowrap"
        >
          engine_search
        </Link>
        <span>/</span>
        <span className="text-text-primary whitespace-nowrap">
          uid_{master.id.substring(master.id.length - 8)}
        </span>
      </nav>
