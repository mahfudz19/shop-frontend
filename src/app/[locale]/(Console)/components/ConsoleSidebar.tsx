...
export default function ConsoleSidebar({ onItemClick }: { onItemClick?: () => void }) {
  ...
  return (
    <aside className="w-64 border-r border-divider bg-background-paper h-full flex flex-col pt-6 overflow-y-auto">
      ...
      <div className="px-6 mb-8">
        <Link href="/" onClick={onItemClick} className="flex items-center gap-2 group">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`${isActive ? "text-white" : "text-text-secondary group-hover:text-primary-main"}`}>
                {item.icon}
              </span>
